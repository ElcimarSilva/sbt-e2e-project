describe("API Tests", { tags: ["@api"] }, () => {
  beforeEach(() => {
    ...
    expect(
      Cypress.env("CYPRESS_CMS_API_TOKEN"),
      "CYPRESS_CMS_API_TOKEN deve estar configurado",
    ).to.exist;
    expect(
      Cypress.env("CYPRESS_CMS_BASE_URL"),
      "CYPRESS_CMS_BASE_URL deve estar configurado",
    ).to.exist;
    expect(
      Cypress.env("CYPRESS_AUTH_BASE_URL"),
      "CYPRESS_AUTH_BASE_URL deve estar configurado",
    ).to.exist;
  });
  ...
  it("Deve buscar vídeos por playlist id 1 (sbt-video-uploader)", () => {
    cy.getVideos(
      "?filters[playlist][id][$eq]=1&pagination[withCount]=true",
    ).then((response) => {
      cy.log(JSON.stringify(response.body));
    ...
    cy.getArticles("?filters[isHighlight][$eq]=false").then((response) => {
      ...
      if (response.body.data.length > 0) {
        response.body.data.forEach((article) => {
          if (
            article.attributes &&
            article.attributes.hasOwnProperty("isHighlight")
          ) {
